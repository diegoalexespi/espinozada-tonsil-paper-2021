...
require(readr)
require(readxl)
require(pochi) #if not installed: devtools::install_github("diegoalexespi/pochi")
...
B.combined <- IntegrateData(anchorset = B.anchors, normalization.method = "SCT", dims = 1:40, verbose = TRUE) 
...
We next use a custom function, `BuildAnnoyUMAP` which calls the 'uwot' package implementation of UMAP. Briefly, this builds an approximate nearest neighbors graph (kNN) on the PCA or Harmony space (100 dimensions) using the annoy package (with cosine distance metric). We store the kNN graph, the cosine distance graph, and derive a shared-nearest-neighbor (SNN) graph from the kNN graph that is stored as well. We also use this function to produce the UMAP dimensionality reduction coordinates, and we will keep the SNN for use in clustering downstream.
```{r UMAP & kNN/SNN, eval = rerun_analysis, include = TRUE}
load(file = paste0(parent.directory, "sct_PCA.RData"))
dims.to.use <- 1:35
B.combined <- pochi::BuildAnnoyUMAP(object = B.combined,
                                    reduction = "pca",
                                    dims = dims.to.use,
...
load(file = paste0(parent.directory, "sct_clustered.RData"))
cluster.table <- readxl::read_excel("sct_seurat_3_cluster_assignment.xlsx")
cluster.levels <- c("Naive 1",
                    "Naive 2",
                    "Naive IER2",
                    "Naive IFN1",
                    "B EIF5A",
...
cluster.table <- cluster.table %>%
    dplyr::mutate(cluster_names = factor(cluster_names, levels = cluster.levels)) %>%
    dplyr::arrange(cluster_names) %>%
    dplyr::mutate(new_clusters_num = factor(new_clusters_num, levels = new_clusters_num))

B.combined$new_clusters_nums <- factor(paste0("C", as.character(B.combined$annoy_nn_snn_res.1.3)), levels = cluster.table$new_clusters_num)
...
B.combined$cluster_names <- plyr::mapvalues(as.character(B.combined$new_clusters_nums),
                                            from = as.character(cluster.table$new_clusters_num),
                                            to = as.character(cluster.table$cluster_names))
B.combined$cluster_names <- factor(B.combined$cluster_names, levels = levels(cluster.table$cluster_names))
...
```{r split dataset, eval = TRUE, include = TRUE, results = "hide"}
