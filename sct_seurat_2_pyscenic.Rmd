...
B.combined <- pochi::RunAUCell(B.combined, assay = "RNA", ranking.save = TRUE, genesets = regulon_list, slot = "counts",auc_assay_name = "regulons")
...
B.combined <- BuildAnnoyUMAP(B.combined,
                             assay = "regulons",
                             features = rownames(B.combined[["regulons"]]),
                             reduction = NULL,
                             slot = "scale.data",
                             n.neighbors = 30,
                             prune.snn = 1/30,
                             metric = "euclidean",
                             reduction.name = "regulon_UMAP",
...
my_genesets_dataframe <- ReadGMT("intermediate/aucell/genesets.gmt")
...
hm <- clusterProfiler::read.gmt("molsigdb/h.all.v7.2.symbols.gmt")
...
Here we determine which gene set AUCell scores are differentially expressed (Wilcoxon rank-sum test) between clusters.
...
save(diff_exp_regulons, diff_exp_genesets, diff_exp_hms, B.combined, file = paste0(parent.directory, "sct_reguloned.RData"))
```
...
dimplot_regulon_clusters <- DimPlot(B.combined, reduction  = "pca_UMAP", group.by =  "regulon_annoy_nn_snn_res.0.8", label = TRUE, label.size = 5, shuffle = TRUE)+scale_color_manual(values = c(RColorBrewer::brewer.pal(12, "Set3")))
regulon_dimplot_regulon_clusters <- DimPlot(B.combined, reduction  = "regulon_UMAP", group.by =  "regulon_annoy_nn_snn_res.0.8", label = TRUE, label.size = 5, shuffle = TRUE)+scale_color_manual(values = c(RColorBrewer::brewer.pal(12, "Set3")))
dimplot_regulon_clusters + regulon_dimplot_regulon_clusters
```

### B.7 Alluvial plots 1
We next plot the regulon-derived clusters on the existing RNA-derived clusters to get a better idea of the compositional relationship of the two sets of clusters.
...
B.combined@meta.data %>%
  ...
  scale_fill_manual(values = c(RColorBrewer::brewer.pal(12, "Set3")))

```

### B.8 Alluvial plots 2
More plotting of regulon-derived clusters on the existing RNA-derived clusters to get a better idea of the compositional relationship of the two sets of clusters.
...
We next plot the differentially expressed gene sets (comparison between clusters using a one-vs-all approach) in heat maps across the RNA-based clusters, with stars denoting "differentially expressed" gene sets (AUC > 0.9 or AUC > 0.8).
```{r heatmap, eval = TRUE, fig.width=12, fig.height=9}
pochi::DoStarHeatmap(B.combined, assay = "regulons", diff_exp_results = diff_exp_regulons, group.by = "cluster_names", scale_rows = TRUE, auc_choice = 0.9, cluster_cols = FALSE)
pochi::DoStarHeatmap(B.combined, assay = "genesetAUC", diff_exp_results = diff_exp_genesets, group.by = "cluster_names", scale_rows = TRUE, auc_choice = 0.8, plot_all = TRUE, cluster_cols = FALSE)
pochi::DoStarHeatmap(B.combined, assay = "hmAUC", diff_exp_results = diff_exp_hms, group.by = "cluster_names", scale_rows = TRUE, auc_choice = 0.8, plot_all = FALSE, cluster_cols = FALSE)

```


### B.10 Heatmap of markers based on regulon clusters
We next plot the differentially expressed gene sets (comparison between clusters using a one-vs-all approach) in heat maps across the regulon-based clusters, with stars denoting "differentially expressed" gene sets (AUC > 0.8)
...
pochi::DoStarHeatmap(B.combined, assay = "regulons", diff_exp_results = diff_exp_regulons_by_regulons, group.by = "regulon_annoy_nn_snn_res.0.8", scale_rows = TRUE, auc_choice = 0.8, plot_all = FALSE)
```
